...
public enum FlightParticlesBlackLightning implements FlightParticleRenderer
{
    INSTANCE;
    ...
    public void doParticles(LivingEntity entity, HeroIteration iter, HeroRenderer renderer, Random rand, Vec3 pos, double d, double d1, double yaw, float scale, boolean clientPlayer, boolean firstPerson)
    {
        ...
        for (int side = 0; side < 2; ++side)
        {
            if (side == 0 || !SHData.AIMING.get(entity))
            {
                ...
                {
                    scale(SHParticlesClient.spawnParticleClient(SHParticleType.BLUE_FLAME, vec3.xCoord + (rand.nextDouble() * 2 - 1) / 10, vec3.yCoord, vec3.zCoord + (rand.nextDouble() * 2 - 1) / 10, (rand.nextDouble() * 2 - 1) / 20 * scale, -0.25 * scale, (rand.nextDouble() * 2 - 1) / 20 * scale), scale);
                }
            }
        }
    }
